...
class ChangePhoneNumber extends StatefulWidget {
  ProviderProfile providerProfile;
   ChangePhoneNumber({Key? key, required this.providerProfile}) : super(key: key);
...
class _ChangePhoneNumberState extends State<ChangePhoneNumber> {
  ...
  Widget build(BuildContext context) {
              ...
              validator: (value) {
                if (value == null || value.length < 9) {
                  if (value!.length > 1) {
                    ...
                    if (!widget.providerProfile.myBoolWidget && value.length < 3) {
                      return "kodError".tr();
                    } else {
                      return "kodLength".tr();
                    }
                  ...
                  for (var element in MyWidgets.checkTelephoneCompanyCode) {
                    if (element.contains(kod)) {
    widget.providerProfile.myBoolWidget = true;
    widget.providerProfile.boolButtonCol1(boolValue: true);
                      break;
                    } else {
    widget.providerProfile.boolButtonCol1(boolValue: false);
    widget.providerProfile.myBoolWidget = false;
                    }
                  }
    ...
    widget.providerProfile.boolButtonCol1(boolValue: true);
              ...
              MaterialButton(child: MyWidgets.robotoFontText(
                  text:"access".tr(),
                  textColor: MyColors.appColorWhite()),
                onPressed: (){
                widget.providerProfile.changePhoneNumber();
                },
